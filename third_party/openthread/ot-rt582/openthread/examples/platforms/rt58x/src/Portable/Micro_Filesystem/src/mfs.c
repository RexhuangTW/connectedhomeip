...
{
    _memcpy_ram((uint8_t *)ram_addr, (uint8_t *)RT58X_FLASH_BASE_ADDR + fc_addr, size);
}
...
void filesystem_reset(void)
{
    while (flash_check_busy());
    //taskENTER_CRITICAL();
    flash_erase(FLASH_ERASE_PAGE, RT58X_FLASH_BASE_ADDR + MFS_START_ADDRESS);
    //taskEXIT_CRITICAL();
...
fs_err_t file_system_init(void)
{
    ...
    do
    {
        ...
        else
        {
            ...
            if (flash_erase(FLASH_ERASE_PAGE, RT58X_FLASH_BASE_ADDR + MFS_START_ADDRESS) != 0)
            {
