...
static void host_check_queue(void)
{
        ...
        (sys_queue_remaining_size(&g_host_to_app_handle) != QUEUE_HOST_TO_APP_EVENT))
    {
        ...
        if ((sys_queue_remaining_size(&g_host_acl_tx_handle) >= 5) &&
            (sys_queue_remaining_size(&g_host_to_app_handle) >= 2))
        {
    ...
    if (recv_msg_time != SYS_ARCH_TIMEOUT)
    {
        ...
        default:
            BLE_PRINTF(BLE_DEBUG_ERR, "[BLE_DEBUG_HOST] %s recv unknow type %d (TLV %p(%d))\n",
                       sys_current_thread_name(), p_mblk->primitive,
                       p_mblk, p_mblk->length + sizeof(MBLK));
...
void ble_host_timer_cb(TimerHandle_t xTimer)
{
    ...
    for (i = 0; i < MAX_CONN_NO_APP; i++) // hostId
    {
        for (j = 0; j < TOTAL_TIMER_EVENT; j++)
        {
            ...
            {
                no_evt = 0;
                if (((timer_count - (host_timer + j)[i * TOTAL_TIMER_EVENT].current_time)) > (host_timer + j)[i * TOTAL_TIMER_EVENT].timeout_base)
                {
                    ...
                    case TIMER_EVENT_CLIENT_DB_PARSING:
                    {
                        ...
                        if (p_mblk != NULL)
                        {
                            ...
                            p_evt_param->event_param.ble_evt_att_gatt.param.ble_evt_att_db_parse_complete.result = BLE_ERR_CODE_TIMEOUT;
...
int8_t ble_host_subsystem_init(void)
{
    ...
    do
    {
        ...
        if (sys_sem_new(&g_host_rx_data_sem, (QUEUE_HOST_RX - CONFIG_RX_CMD_SEMAPHORE_SIZE - 10), (QUEUE_HOST_RX - CONFIG_RX_CMD_SEMAPHORE_SIZE - 10)) != ERR_OK)
        {
        ...
        ble_cmd_transport_task = sys_task_new("BLE_CMD_TRANSPORT",
                                              (TaskFunction_t)ble_cmd_transport_handle,
                                              NULL,
                                              CONFIG_CMD_TRANSPORT_STACK_SIZE,
                                              CONFIG_CMD_TRANSPORT_PRIORITY);
        ...
        ble_host_subsystem_task = sys_task_new("HOST_SUBSYSTEM",
                                               (TaskFunction_t)ble_host_subsystem_handle,
                                               NULL,
                                               CONFIG_HOST_SUBSYSTEM_STACK_SIZE,
                                               CONFIG_HOST_SUBSYSTEM_PRIORITY);
