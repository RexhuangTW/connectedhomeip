...
{
    //DisplayOnly,                  Display YesNo,              Keyboard-Only,                      NoInputNoOutput,            KeyboardDisplay
    {
        STK_GEN_MTHD_JUST_WORKS,    STK_GEN_MTHD_JUST_WORKS,    STK_GEN_MTHD_PASSKEY_ENTRY_DISP,    STK_GEN_MTHD_JUST_WORKS,    STK_GEN_MTHD_PASSKEY_ENTRY_DISP     //DisplayOnly
    },
    {
        STK_GEN_MTHD_JUST_WORKS,    STK_GEN_MTHD_JUST_WORKS,    STK_GEN_MTHD_PASSKEY_ENTRY_DISP,    STK_GEN_MTHD_JUST_WORKS,    STK_GEN_MTHD_PASSKEY_ENTRY_DISP     //Display YesNo
    },
    {
        STK_GEN_MTHD_PASSKEY_ENTRY, STK_GEN_MTHD_PASSKEY_ENTRY, STK_GEN_MTHD_PASSKEY_ENTRY,         STK_GEN_MTHD_JUST_WORKS,    STK_GEN_MTHD_PASSKEY_ENTRY          //Keyboard-Only
    },
    {
        STK_GEN_MTHD_JUST_WORKS,    STK_GEN_MTHD_JUST_WORKS,    STK_GEN_MTHD_JUST_WORKS,            STK_GEN_MTHD_JUST_WORKS,    STK_GEN_MTHD_JUST_WORKS             //NoInputNoOutput
    },
    {
        STK_GEN_MTHD_PASSKEY_ENTRY, STK_GEN_MTHD_PASSKEY_ENTRY, STK_GEN_MTHD_PASSKEY_ENTRY_DISP,    STK_GEN_MTHD_JUST_WORKS,    STK_GEN_MTHD_PASSKEY_ENTRY         //KeyboardDisplay
    }
};

const uint8_t IRK_FIXED[][16] =
{
    {
        //0xB5, 0xB7, 0xE1, 0x48,
        //0xAC, 0x7E, 0x3B, 0x91,
...
int8_t Prcss_BLE_CODE_SMP_Pairing_Request(void)
{
    ...
    if ((ACL_data->PDU.smp_op_pairing_response.AuthReq.MITM | (app_prefer_smp_format.para.pairing_response.AuthReq.MITM & MITM_PROTECTION_YES)) == 0)
    {
        p_host_current->SMP_STKGenMethod = STK_GEN_MTHD_JUST_WORKS;
    }
    else
    {
        p_host_current->SMP_STKGenMethod = SEL_STK_GEN_MTHD[app_prefer_smp_format.para.pairing_response.IOCapability][p_host_current->Remote_IOCapab];
    }

    if ((ACL_data->PDU.smp_op_pairing_request.MaximumEncryptionKeySize > 6) && (ACL_data->PDU.smp_op_pairing_request.MaximumEncryptionKeySize < 17))
    {
        p_host_current->MaxEncKeySize = app_prefer_smp_format.para.pairing_response.MaximumEncryptionKeySize >= ACL_data->PDU.smp_op_pairing_request.MaximumEncryptionKeySize ? ACL_data->PDU.smp_op_pairing_request.MaximumEncryptionKeySize : app_prefer_smp_format.para.pairing_response.MaximumEncryptionKeySize;

        memset(p_host_current->PeerLTK_Passkey, 0, SIZE_AES_KEY);

        if (setBLE_ConnTxData(p_host_current->LL_ConnID, (uint8_t *)&app_prefer_smp_format, (app_prefer_smp_format.length + SIZE_BASIC_L2CAP_HEADER)) == ERR_OK)
        {
...
int8_t Prcss_BLE_CODE_SMP_Pairing_Response(void)
{
    ...
    do
    {
        ...
        memcpy(&p_host_current->Remote_IOCapab, &ACL_data->PDU.smp_op_pairing_response.IOCapability, 6);
        if ((ACL_data->PDU.smp_op_pairing_response.AuthReq.MITM | (app_prefer_smp_format.para.pairing_request.AuthReq.MITM & MITM_PROTECTION_YES)) == 0)
        {
            p_host_current->SMP_STKGenMethod = STK_GEN_MTHD_JUST_WORKS;
        }
        else
        {
            p_host_current->SMP_STKGenMethod = SEL_STK_GEN_MTHD[app_prefer_smp_format.para.pairing_request.IOCapability][p_host_current->Remote_IOCapab];
        }

        if ((ACL_data->PDU.smp_op_pairing_response.MaximumEncryptionKeySize > 6) && (ACL_data->PDU.smp_op_pairing_response.MaximumEncryptionKeySize < 17))
        {
            p_host_current->MaxEncKeySize = app_prefer_smp_format.para.pairing_request.MaximumEncryptionKeySize >= ACL_data->PDU.smp_op_pairing_response.MaximumEncryptionKeySize ? ACL_data->PDU.smp_op_pairing_response.MaximumEncryptionKeySize : app_prefer_smp_format.para.pairing_request.MaximumEncryptionKeySize;
...
int8_t Prcss_BLE_CODE_SMP_Master_Identification(void)
{
    ...
    do
    {
        ...
        if (p_host_current->Role == BLE_GAP_ROLE_CENTRAL)
        {
            if (((p_host_current->Remote_ResponserKeyDistrib & app_prefer_smp_format.para.pairing_request.ResponderKeyDistribution) & (KEY_DISTRIBUTION_IDKEY_1 | KEY_DISTRIBUTION_SIGN_1)) == 0)
            {
                i = p_host_current->Remote_InitiatorKeyDistrib & app_prefer_smp_format.para.pairing_request.InitiatorKeyDistribution;
                if ((i & KEY_DISTRIBUTION_ENCKEY_1))
                {
                    memset(p_host_current->OwnLTK, 0, (SIZE_AES_KEY - p_host_current->MaxEncKeySize));
                    setBLE_ConnTxData_SMP(g_l2cap_buffer.conn_handle, CODE_SMP_ENCRYPTION_INFORMATION, (uint8_t *)p_host_current->OwnLTK, SIZE_AES_KEY);
                    setBLE_ConnTxData_SMP(g_l2cap_buffer.conn_handle, CODE_SMP_MASTER_IDENTIFICATION, (uint8_t *)p_host_current->OwnRAND, (SIZE_SMP_EDIV + SIZE_SMP_RAND));
                ...
                if ((i & KEY_DISTRIBUTION_IDKEY_1))
                {
                    ...
                    setBLE_ConnTxData_SMP(g_l2cap_buffer.conn_handle, CODE_SMP_IDENTITY_INFORMATION, (uint8_t *)p_host_current->OwnIRK, SIZE_SMP_IRK);
                    ...
                    setBLE_ConnTxData_SMP(g_l2cap_buffer.conn_handle, CODE_SMP_IDENTITY_ADDRESS_INFORMATION, &inverse_addr[0], (1 + BLE_ADDR_LEN));
                }
                if ((i & KEY_DISTRIBUTION_SIGN_1))
                {
                    setBLE_ConnTxData_SMP(g_l2cap_buffer.conn_handle, CODE_SMP_SIGNING_INFORMATION, (uint8_t *)CSRK_FIXED, SIZE_SMP_CSRK);
                }
        ...
        {
            if (((p_host_current->Remote_InitiatorKeyDistrib & app_prefer_smp_format.para.pairing_response.InitiatorKeyDistribution) & (KEY_DISTRIBUTION_IDKEY_1 | KEY_DISTRIBUTION_SIGN_1)) == 0)
            {
...
int8_t Prcss_BLE_CODE_SMP_Identity_Address_Information(void)
{
    ...
    do
    {
        ...
        if (p_host_current->Role == BLE_GAP_ROLE_CENTRAL)
        {
            if (((p_host_current->Remote_ResponserKeyDistrib & app_prefer_smp_format.para.pairing_request.ResponderKeyDistribution) & KEY_DISTRIBUTION_SIGN_1) == 0)
            {
                i = p_host_current->Remote_InitiatorKeyDistrib & app_prefer_smp_format.para.pairing_request.InitiatorKeyDistribution;
                if ((i & KEY_DISTRIBUTION_ENCKEY_1))
                {
        ...
        {
            if (((p_host_current->Remote_InitiatorKeyDistrib & app_prefer_smp_format.para.pairing_response.InitiatorKeyDistribution) & (KEY_DISTRIBUTION_SIGN_1)) == 0)
            {
                p_mblk = get_msgblks_L1(sizeof(MHS_ATT_Authe_Carry_On_Para));
                if (p_mblk == NULL)
                {
...
int8_t Prcss_BLE_CODE_SMP_Security_Request(void)
{
    ...
    do
    {
        ...
        if ((p_host_current->IsEncryption == DISABLE) && (p_host_current->Smp_Phase == 0) && (p_host_current->state_authe < HS_ATE_S2))
        {
            p_mblk = get_msgblks_L1(sizeof(MHS_ATT_Authe_Carry_On_Para));
            if (p_mblk == NULL)
            {
...
void bhc_prcss_encrpytion_change(ble_hci_evt_param_encrypt_change_t *p_data)
{
    ...
    if (p_data->encrypt_enabled == HCI_ENCRPT_EN_AES_CCM_LE)
    {
        if (p_host_current->KeyType == SMP_KEY_USE_STK)
        {
            ...
            {
                i = p_host_current->Remote_ResponserKeyDistrib & app_prefer_smp_format.para.pairing_response.ResponderKeyDistribution;
                if ((i & KEY_DISTRIBUTION_ENCKEY_1))
                {
                    memset(p_host_current->OwnLTK, 0, (SIZE_AES_KEY - p_host_current->MaxEncKeySize));
                    setBLE_ConnTxData_SMP(p_host_current->LL_ConnID, CODE_SMP_ENCRYPTION_INFORMATION, (uint8_t *)p_host_current->OwnLTK, SIZE_AES_KEY);
                    setBLE_ConnTxData_SMP(p_host_current->LL_ConnID, CODE_SMP_MASTER_IDENTIFICATION, (uint8_t *)p_host_current->OwnRAND, (SIZE_SMP_EDIV + SIZE_SMP_RAND));
                    p_host_current->KeyType = SMP_KEY_USE_LTK;
                }
                if ((i & KEY_DISTRIBUTION_IDKEY_1))
                {
                    ...
                    setBLE_ConnTxData_SMP(p_host_current->LL_ConnID, CODE_SMP_IDENTITY_INFORMATION, (uint8_t *)p_host_current->OwnIRK, SIZE_SMP_IRK);
                    ble_gap_device_identity_address_get(&identity_addr);
                    inverse_addr[6] = identity_addr.addr_type;
                    memcpy_inv((uint8_t *)&inverse_addr[0], (uint8_t *)&identity_addr.addr[0], BLE_ADDR_LEN);
                    setBLE_ConnTxData_SMP(p_host_current->LL_ConnID, CODE_SMP_IDENTITY_ADDRESS_INFORMATION, &inverse_addr[0], (1 + BLE_ADDR_LEN));
                }
                if ((i & KEY_DISTRIBUTION_SIGN_1))
                {
                    setBLE_ConnTxData_SMP(p_host_current->LL_ConnID, CODE_SMP_SIGNING_INFORMATION, (uint8_t *)CSRK_FIXED, SIZE_SMP_CSRK);
                }
                i = p_host_current->Remote_InitiatorKeyDistrib & app_prefer_smp_format.para.pairing_response.InitiatorKeyDistribution;
                if (i == 0)
                {
                    p_mblk = get_msgblks(sizeof(MHS_ATT_Authe_Carry_On_Para));
                    while (p_mblk == NULL)
            ...
            {
                i = p_host_current->Remote_ResponserKeyDistrib & app_prefer_smp_format.para.pairing_request.ResponderKeyDistribution;
                if (i == 0)
                {
                    i = p_host_current->Remote_InitiatorKeyDistrib & app_prefer_smp_format.para.pairing_request.InitiatorKeyDistribution;
                    if ((i & KEY_DISTRIBUTION_ENCKEY_1))
        ...
        if (p_host_current->SMP_STKGenMethod != STK_GEN_MTHD_JUST_WORKS)       //STK_GEN_MTHD_PASSKEY_ENTRY or STK_GEN_MTHD_PASSKEY_ENTRY_DISP
        {
...
ble_err_t bhc_sm_security_request(uint8_t host_id)
{
    ...
    if ((p_le_current->IsEncryption == DISABLE) && ((p_le_current->state_authe == HS_ATE_S0) || (p_le_current->state_authe == HS_ATE_S253)))
    {
