...
static int8_t Prcss_L2CAP_CID_ATTRIBUTE_PROTOCOL(void)
{
    ...
    if ((pkt_hdr->opcode & 0xE0) == 0)
    {
        ...
        {
            err_code =
                ATT_Request_ERROR_RESPONSE(g_l2cap_buffer.conn_handle, pkt_hdr->opcode, 0, ERR_CODE_ATT_REQUEST_NOT_SUPPORTED);
        }
    }
    else
    {
...
static int8_t Prcss_L2CAP_CID_LE_L2CAP_SIGNALING_CHANNEL(void)
{
        ...
        if (ACL_data->PDU.L2CAPR_SIGNL_OP_Connection_Parameter_Update_Response.Result == 0x01)
        {
            ...
            if (p_queue_param != NULL)
            {
                ...
                p_evt_param->event_param.ble_evt_gap.param.evt_conn_param_update.status =
                    ACL_data->PDU.L2CAPR_SIGNL_OP_Connection_Parameter_Update_Response.Result;
    ...
    case CODE_SIGNL_CMD_CONNECTION_PARAMETER_UPDATE_REQUEST: {
        ...
        {
            if ((ACL_data->PDU.L2CAPR_SIGNL_OP_Connection_Parameter_Update_Request.Intervalmin < 6 ||
                 ACL_data->PDU.L2CAPR_SIGNL_OP_Connection_Parameter_Update_Request.Intervalmin > 3200) ||
                (ACL_data->PDU.L2CAPR_SIGNL_OP_Connection_Parameter_Update_Request.Intervalmax < 6 ||
                 ACL_data->PDU.L2CAPR_SIGNL_OP_Connection_Parameter_Update_Request.Intervalmax > 3200) ||
                (ACL_data->PDU.L2CAPR_SIGNL_OP_Connection_Parameter_Update_Request.Intervalmin >
                 ACL_data->PDU.L2CAPR_SIGNL_OP_Connection_Parameter_Update_Request.Intervalmax) ||
                (ACL_data->PDU.L2CAPR_SIGNL_OP_Connection_Parameter_Update_Request.SlaveLatency > 500) ||
                (ACL_data->PDU.L2CAPR_SIGNL_OP_Connection_Parameter_Update_Request.TimeoutMultiplier < 10 ||
                 ACL_data->PDU.L2CAPR_SIGNL_OP_Connection_Parameter_Update_Request.TimeoutMultiplier > 3200) ||
                ((ACL_data->PDU.L2CAPR_SIGNL_OP_Connection_Parameter_Update_Request.TimeoutMultiplier << 2) <
                 ((1 + ACL_data->PDU.L2CAPR_SIGNL_OP_Connection_Parameter_Update_Request.SlaveLatency) *
                  ACL_data->PDU.L2CAPR_SIGNL_OP_Connection_Parameter_Update_Request.Intervalmax)))
            {
            ...
            else
            {
                ...
                if (p_hci_msg != NULL)
                {
                    ...
                    memcpy((uint8_t *) &p_hci_data->conn_interval_min,
                           (uint8_t *) &ACL_data->PDU.L2CAPR_SIGNL_OP_Connection_Parameter_Update_Request.Intervalmin, 8);
            ...
            if (err_code != ERR_MEM)
            {
                ...
                ACL_dataTx->PDU.L2CAPR_SIGNL_OP_Connection_Parameter_Update_Response.Code =
                    CODE_SIGNL_CMD_CONNECTION_PARAMETER_UPDATE_RESPONSE;
                ACL_dataTx->PDU.L2CAPR_SIGNL_OP_Connection_Parameter_Update_Response.Identifier =
                    ACL_data->PDU.L2CAPR_SIGNL_OP_Connection_Parameter_Update_Request.identifier;
                ACL_dataTx->PDU.L2CAPR_SIGNL_OP_Connection_Parameter_Update_Response.Length = 2;
                err_code = setBLE_ConnTxData(g_l2cap_buffer.conn_handle, (uint8_t *) ACL_dataTx,
                                             (ACL_dataTx->Length_PDU + SIZE_BASIC_L2CAP_HEADER));
                if (p_hci_msg != NULL)
                {
                    if (err_code == ERR_OK)
                    {
        ...
        else
        {
            ...
            ACL_dataTx->PDU.L2CAPR_SIGNL_OP_Command_Reject_Response.identifier =
                ACL_data->PDU.L2CAPR_SIGNL_OP_Connection_Parameter_Update_Request.identifier;
