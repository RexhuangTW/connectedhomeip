 ...
 **************************************************************************/
#define ATTR_TEMP_DEF (void *) 0, (void *) 0, 0, 0, 0, attr_null_access,

const ble_att_param_t ATT_NULL_INVALID = { ATTR_TEMP_DEF };
 ...
 */
ble_err_t ble_svcs_common_info_index_query(uint8_t host_id, ble_gatt_role_t role, uint8_t maxCount, ble_svcs_common_info_t * p_info,
                                           uint8_t * p_index)
{
    uint8_t i;
 ...
 */
ble_err_t ble_svcs_common_init(uint8_t host_id, ble_gatt_role_t role, uint8_t maxCount, ble_svcs_common_info_t * p_info,
                               uint8_t * p_index, uint8_t * p_count)
{
    // check host id
    if ((host_id == BLE_HOSTID_RESERVED) || (host_id >= BLE_SUPPORT_NUM_CONN_MAX))
    {
...
ble_err_t ble_svcs_handles_mapping_get(ble_gatt_handle_table_param_t * p_param)
{
    ...
    status    = BLE_ERR_OK;
    p_ble_tlv = sys_malloc(sizeof(ble_tlv_t) + sizeof(ble_gatt_handle_table_param_t));
...
ble_err_t ble_svcs_handle_cccd_write_req(uint8_t * p_data, uint16_t length, uint16_t * p_cccd)
{
    ...
    if ((cccdValue != BLEGATT_CCCD_NONE) && (cccdValue != BLEGATT_CCCD_NOTIFICATION) && (cccdValue != BLEGATT_CCCD_INDICATION) &&
...
ble_err_t ble_svcs_auto_handle_cccd_read_req(ble_evt_att_param_t * p_param, uint16_t cccd_value)
{
    ...
    if ((cccd_value != BLEGATT_CCCD_NONE) && (cccd_value != BLEGATT_CCCD_NOTIFICATION) && (cccd_value != BLEGATT_CCCD_INDICATION) &&
    ...
    case OPCODE_ATT_READ_BY_TYPE_REQUEST: {
        ...
        p_ble_tlv = sys_malloc(sizeof(ble_tlv_t) + sizeof(ble_gatt_data_param_t));
        if (p_ble_tlv != NULL)
        {
...
ble_err_t ble_svcs_auto_handle_read_blob_req(ble_evt_att_param_t * p_param, uint8_t * p_data, uint8_t length)
{
    ...
    if (err_rsp != ERR_CODE_ATT_NO_ERROR)
    {
        ...
        p_ble_tlv = sys_malloc(sizeof(ble_tlv_t) + sizeof(ble_gatt_err_rsp_param_t));
