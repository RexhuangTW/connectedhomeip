...
static void read_gap_appearance_cmd_init(cmd_info_t *this);
static ble_err_t set_cmd(atcmd_item_t *item);
static void test_cmd(atcmd_item_t *item);
static void check_service(ble_evt_att_param_t *p_param, atcmd_item_t *item);

// PUBLIC VARIABLE DECLARE
cmd_info_t read_gap_appearance =
{
    .cmd_name = "+READDEVAP",
    .description = "read gap appearance",
    .init = read_gap_appearance_cmd_init
};
...
static ble_err_t set_cmd(atcmd_item_t *item)
{
    ...
    if (item->param_length == 1)
    {
        ...
        {
            handle_num = ble_info_link[host_id].svcs_info_gaps.server_info.handles.hdl_appearance;
        }
        else
        {
            handle_num = ble_info_link[host_id].svcs_info_gaps.client_info.handles.hdl_appearance;
        }
...
static void check_service(ble_evt_att_param_t *p_param, atcmd_item_t *item)
{
    if (p_param->event == BLESERVICE_GAPS_APPEARANCE_READ_RSP_EVENT && p_param->host_id == host_id)
    {
