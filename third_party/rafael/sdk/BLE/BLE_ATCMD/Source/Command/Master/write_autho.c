...
static void write_autho_cmd_init(cmd_info_t *this);
static ble_err_t set_cmd(atcmd_item_t *item);
static void test_cmd(atcmd_item_t *item);
static void check_service(ble_evt_att_param_t *p_param, atcmd_item_t *item);

// PUBLIC VARIABLE DECLARE
cmd_info_t write_autho =
{
    .cmd_name = "+WRITEAUTHO",
    .description = "write authorisation",
    .init = write_autho_cmd_init
};
...
static ble_err_t set_cmd(atcmd_item_t *item)
{
    ...
    if (item->param_length == 2)
    {
        ...
        uint16_t handle_num = ble_info_link[host_id].svcs_info_atcmd.info.handles.hdl_charac04;
...
static void check_service(ble_evt_att_param_t *p_param, atcmd_item_t *item)
{
    if (p_param->event == BLESERVICE_ATCMD_CHARAC04_WRITE_RSP_EVENT && p_param->host_id == host_id)
    {
