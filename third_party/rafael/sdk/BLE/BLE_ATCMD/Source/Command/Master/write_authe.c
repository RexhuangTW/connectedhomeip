...
static ble_err_t set_cmd(atcmd_item_t *item)
{
    ...
    if (item->param_length == 2)
    {
        ...
        uint16_t handle_num = ble_info_link[host_id].svcs_info_atcmd.info.handles.hdl_charac03;
...
static void check_service(ble_evt_att_param_t *p_param, atcmd_item_t *item)
{
    if (p_param->event == BLESERVICE_ATCMD_CHARAC03_WRITE_RSP_EVENT && p_param->host_id == host_id)
    {
