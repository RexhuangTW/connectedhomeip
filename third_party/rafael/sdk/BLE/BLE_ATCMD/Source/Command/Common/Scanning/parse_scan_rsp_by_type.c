...
static void parse_scan_rsp_by_type_cmd_init(cmd_info_t *this);
static ble_err_t set_cmd(atcmd_item_t *item);
static void test_cmd(atcmd_item_t *item);

// PUBLIC VARIABLE DECLARE
cmd_info_t parse_scan_rsp_by_type =
{
    .cmd_name = "+PARSSCAN",
    .description = "parsing scan response by type",
    .init = parse_scan_rsp_by_type_cmd_init
};
...
static ble_err_t set_cmd(atcmd_item_t *item)
{
    ...
    if (item->param_length == 1)
    {
        ...
        ble_evt_scan_adv_report_t *adv_report = last_scan_rsp_get();
