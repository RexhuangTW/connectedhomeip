...
static void read_con_timeout_cmd_init(cmd_info_t *this);
static ble_err_t set_cmd(atcmd_item_t *item);
static void test_cmd(atcmd_item_t *item);

// PUBLIC VARIABLE DECLARE
cmd_info_t read_con_timeout =
{
    .cmd_name = "+READCONSUPTMO",
    .description = "read connection supervision timeout",
    .init = read_con_timeout_cmd_init
};
...
static ble_err_t set_cmd(atcmd_item_t *item)
{
    ...
    if (item->param_length == 1)
    {
        ...
        //printf(("%u\n", con_param[host_id].supv_timeout);
