...
ble_err_t ble_cmd_phy_controller_init(void)
{
    ...
    status    = BLE_ERR_OK;
    p_ble_tlv = sys_malloc(sizeof(ble_tlv_t) + sizeof(ble_common_controller_info_t));
...
ble_err_t ble_cmd_read_unique_code(ble_unique_code_format_t * p_param)
{
    ...
    status       = BLE_ERR_OK;
    p_read_param = sys_malloc(256);

    do
    {
        if (p_read_param != NULL)
        {
            p_cmp = sys_malloc(sizeof(ble_unique_code_format_t));
            if (p_cmp != NULL)
            {
...
ble_err_t ble_cmd_add_device_to_accept_list(ble_filter_accept_list_t * p_accept_list)
{
    ...
    status    = BLE_ERR_OK;
    p_ble_tlv = sys_malloc(sizeof(ble_tlv_t) + sizeof(ble_filter_accept_list_t));
