...
uint32_t ds_initinal(uint32_t start_addr, uint32_t end_addr, uint32_t size)
{
		...
		#if (DS_ONE_PAGE==ENABLE)
	
			if((end_addr>(start_addr+DATA_SET_ONE_PAGE_SIZE)) || (start_addr<DS_START_ADDRESS))
			 {
					dsinfo.invaild = DS_INVAILD;
					return STATUS_INVALID_PARAM;
			 }
			 
			if(size!=DATA_SET_ONE_PAGE_SIZE)
			{
				dsinfo.invaild = DS_INVAILD;
				return STATUS_INVALID_PARAM;
			}
...
uint32_t ds_pagex_check()
{
	  ...
	  uint32_t page1_satatus,satatus = STATUS_SUCCESS;
	  ...
	  uint32_t page1_satatus,page2_satatus,satatus = STATUS_SUCCESS;
		...
		page1_satatus = ds_erase_check(&ds_search_pagex);
			...
			if(page1_satatus==STATUS_SUCCESS)						//Page1 and Page 2 All 0xFF
			{
			...
			{
				satatus = ds_page_vaild_address(ds_page1_addr,dspack1);	
...
uint32_t ds_write(ds_rw_t* ds_write)
{
	    ...
	    dsmaxaddress = dsinfo.start_address + (dsinfo.page_size<<dsinfo.current_page);
			dswraddress = dsinfo.current_address  + ds_write->len;
			...
			{	
					...
					if((dsinfo.migration_address+ds_write->len)>dsmaxaddress)
					{
						return STATUS_INVALID_REQUEST;
					}
				...
				flash_write_byte(dswraddress,ds_write->type);				while (flash_check_busy()) {;}	dswraddress+=1;
				flash_write_byte(dswraddress,ds_write->len);				while (flash_check_busy()) {;}	dswraddress+=1;
				flash_write_byte(dswraddress,ds_write->len>>8);			while (flash_check_busy()) {;}	dswraddress+=1;
				...
				flash_write_byte(dswraddress,dsinfo.current_sn);		while (flash_check_busy()) {;}	dswraddress+=1;
				flash_write_byte(dswraddress,dsinfo.current_sn>>8);	while (flash_check_busy()) {;}	dswraddress+=1;
				flash_write_byte(dswraddress,dsinfo.current_sn>>16);while (flash_check_busy()) {;}	dswraddress+=1;	
				flash_write_byte(dswraddress,dsinfo.current_sn>>24);while (flash_check_busy()) {;}	dswraddress+=1;
				...
				flash_write_byte(dswraddress,crc_result); while (flash_check_busy()) {;}						dswraddress+=1;
					
				magic_number = DS_MAGIC_NUMBER;	
				flash_write_byte(dswraddress,magic_number);while (flash_check_busy()) {;}						dswraddress+=1;
				flash_write_byte(dswraddress,magic_number>>8); while (flash_check_busy()) {;} 			dswraddress+=1;
...
uint32_t ds_migration(ds_page_t page)
{
		...
		uint32_t 		ds_status;
		uint32_t 		address,end_address,i,j;
		uint8_t  		update_flag,value;
		...
		#if DS_ONE_PAGE==ENABLE
		static 			uint8_t  	flash_data_buf[4096];
		...
		for(i=1;i<=dsinfo.type_max;i++)
		{
				...
				do{
					  ...
					  for(j=0;j<(DS_HEADER_OFFSET+ds_migration_befor.len);j++)
							{
									value = *(uint32_t*)(ds_migration_befor.buf_addr-DS_HEADER_OFFSET+j);
									flash_data_buf[buf_idx] = value;
									buf_idx+=1;
							}
							
						crc_result = ds_cal_crc(&dswr,dsinfo.current_sn);
						
						flash_data_buf[buf_idx] = crc_result;
						buf_idx+=1;
					
						magic_number = DS_MAGIC_NUMBER;
						flash_data_buf[buf_idx] = magic_number;
						flash_data_buf[buf_idx+1] = magic_number>>8;
						buf_idx+=2;
					  dsinfo.migration_address+=buf_idx;
						dsinfo.current_sn+=1;	
						update_flag = 0;		
			...
			for(i=0;i<dsinfo.migration_address;i++)
			{
				value = *(uint32_t*)(flash_data_buf+i);
				flash_write_byte((address+i),value);
				while (flash_check_busy()) {;}
			}
...
uint32_t ds_migration_erase(ds_page_t page)
{
	...
	if((dsinfo.invaild==DS_INVAILD) || (page>DS_PAGE_0))
	{
		return STATUS_INVALID_PARAM;
	}
