/**************************************************************************//**
 * @file     rt569mxb_ble_init.h
 * @version
 * @brief    host layer phy related configure

 ******************************************************************************/

#ifndef __RT569MXB_BLE_INIT_H__
#define __RT569MXB_BLE_INIT_H__
...
#define HAL_CFG                     (HAL_BLE_SUPPORT)
...
#define HAL_CLK_CFG                 (HAL_16M_CLK_SUPPORT)
...
#define RFK_FREQ_MAX                (4)
#else
#define RFK_FREQ_MAX                (1)
#endif
#if (RF_MCU_CHIP_TYPE == RF_MCU_TYPE_FPGA_RFT3)
#define MDM_CFG_SIZE                (0x70)
#define RF_CFG_SIZE                 (0x90)
#define PMU_CFG_OFFSET              (0x18)
#define PMU_CFG_SIZE                (0x70 - PMU_CFG_OFFSET)
#elif ((RF_MCU_CHIP_TYPE == RF_MCU_TYPE_ASIC) || (RF_MCU_CHIP_TYPE == RF_MCU_TYPE_FPGA_RFM0))
#define MDM_CFG_SIZE                (0x70)
#define RF_CFG_SIZE                 (0xF0)
#define PMU_CFG_OFFSET              (0x18)
#define PMU_CFG_SIZE                (0x100 - PMU_CFG_OFFSET)
#endif
#define MEM_MAX                     (8)
...
typedef struct HAL_HW_CFG
{
    ...
    uint16_t    rfkBand;                        // RFK band, 2.4G, SubG-915, SubG-868, SubG-433, SubG-315
...
typedef struct HAL_HW_PARA_IND
{
...
#if (HALRF_CFG & HALRF_SUBG_SUPPORT)
    uint8_t     symbSyncAlpha1ZigbeeSubg;
#endif
#endif
...
#if (HALRF_CFG & HALRF_SUBG_SUPPORT)
    uint16_t    despreadSoftThZigbeeSubg;
#endif
    uint8_t     chipValueBoundZigbee;
#if (HALRF_CFG & HALRF_SUBG_SUPPORT)
    uint8_t     chipValueBoundZigbeeSubg;
#endif
    uint8_t     btIdxZigbee;
    ...
    uint8_t     vgaGainSubG;
#endif
    uint8_t     rssiOffset[RFK_FREQ_MAX];
...
typedef struct HAL_HW_TAB_IND
{
    ...
    uint8_t     rfTabOpPwCp[RF_FREQ_MAX][IDX_MAX];
    uint8_t     rfTabOpSelRz[RF_FREQ_MAX][IDX_MAX];
    uint8_t     rfTabOpSelCz[RF_FREQ_MAX][IDX_MAX];
    uint8_t     rfTabOpSelCp[RF_FREQ_MAX][IDX_MAX];
    ...
    uint8_t     rfTabTxpGain[TX_PWR_2G_MAX][RF_TAB_TXP_GAIN_LEN];
#endif
#if (HALRF_CFG & HALRF_SUBG_SUPPORT)
    uint8_t     rfTabTxpGainSubg[RF_FREQ_MAX][RF_TAB_TXP_GAIN_SUBG_LEN];
#endif

    /* SubG TXP */
#if (HALRF_CFG & HALRF_SUBG_SUPPORT)
    uint8_t     rfTabTxpSubg[RF_FREQ_MAX];
#endif
...
#if (HALRF_CFG & HALRF_SUBG_SUPPORT)
    uint8_t     phyTabRxFirS2ZigbeeSubg[PHY_TAB_RX_FIR_S2_LEN];
#endif
#endif
#if (HAL_CFG & HAL_SLINK_SUPPORT)
    ...
    /* Gain table */
    uint8_t     phyTabLNAGain[RFK_FREQ_MAX][PHY_TAB_LNA_GAIN_LEN];
