...
uint8_t acl_txdata_memory[251 + 7];
#endif
...
uint8_t check_hci_acl_data_queue(void)
{
    ...
    uint8_t send = FALSE;
    static uint16_t ble_comm_tx_sn = 0;
    ...
    if (sys_arch_queue_tryrecv(&g_hci_tx_acl_handle, &p_hci_message) != SYS_ARCH_TIMEOUT)
    {
        if (p_hci_message->msg_type.hci_tx_acl_data.transport_id == BLE_TRANSPORT_HCI_ACL_DATA)
        {
            ...
            get_acl_data_from_msgblks(&acl_txdata_memory[7], (MBLK *)p_hci_message->msg_type.hci_tx_acl_data.p_data);

            hci_data_length = p_hci_message->msg_type.hci_tx_acl_data.length
...
uint8_t check_hci_cmd_queue(void)
{
    ...
    if (sys_arch_queue_tryrecv(&g_hci_tx_cmd_handle, &p_hci_message) != SYS_ARCH_TIMEOUT)
    {
        ...
        {
            hci_command_length = p_hci_message->msg_type.hci_command.length
...
void check_task_common_queue(void)
{
    ...
    if (msg_waiting_time != SYS_ARCH_TIMEOUT)
    {
        ...
        {
            /*forward cmd to cmd queue*/
            if (sys_queue_send_with_timeout(&g_pci_tx_cmd_handle, &rf_fw_comm_msg.param_type.pci_msg.param_type.p_pci_msg, TASK_DELAY_MS))
            {
                HPCI_PRINTF(PCI_DEBUG_INFO, "[PCI_DEBUG_INFO] pci send cmd fail\n");
        ...
        {
            /*forward data to data queue*/
            if (sys_queue_send_with_timeout(&g_pci_tx_data_handle, &rf_fw_comm_msg.param_type.pci_msg.param_type.p_pci_msg, TASK_DELAY_MS))
            {
