...
#define CONFIG_RX_TASK_STACK_SIZE (256)
...
#define NUM_QUEUE_RX_COMMON ((NUM_HW_RX_DATA_Q + NUM_HW_RX_EVENT_Q) << 1)
...
void rf_fw_isr_hander(uint8_t interrupt_status)
{
    ...
    {
        isr_msg.msg_tag = ISR_MSG_RX_DATA;
        sys_queue_send_from_isr(&g_rx_common_queue_handle, &isr_msg);
    }
...
void hci_reassemble_acl_data(uint8_t *acl_pkt_ptr)
{
    ...
    if (pb_flag == START_FRAME)
    {
        ...
        // packet total length
        reassemble_acl_data[idx].total_data_length = data[0] + (data[1] << 8);

        if (reassemble_acl_data[idx].total_data_length == 0)
        {
            HPCI_PRINTF(HCI_DEBUG_ERR, "[HCI_DEBUG_ERR] invalid L2CAP header!!!\n");
        }

        if (reassemble_acl_data[idx].total_data_length < L2CAP_HEADER_LENGTH_MAX_SIZE)
        {
        ...
        {
            HPCI_PRINTF(HCI_DEBUG_ERR, "[HCI_DEBUG_ERR] invalid l2cap length %d\n", reassemble_acl_data[idx].total_data_length);
        }
    }
    ...
    if (reassemble_acl_data[idx].rcvd_data_length == (reassemble_acl_data[idx].total_data_length + L2CAP_HEADER_SIZE))
    {
        ...
        {
            HPCI_PRINTF(HCI_DEBUG_INFO, "[HCI_DEBUG_INFO] host data sem not available\n");
            Lpm_Low_Power_Mask(LOW_POWER_MASK_BIT_TASK_HCI);
...
void hci_rx_event(uint8_t *cmd_ptr, uint8_t cmd_length)
{
    ...
    {
        data_len = hci_message_ptr->msg_type.hci_event.length + sizeof(host_rx_event_param_t);
        ...
        {
            HPCI_PRINTF(HCI_DEBUG_INFO, "[HCI_DEBUG_INFO] host cmd sem not available\n");
            Lpm_Low_Power_Mask(LOW_POWER_MASK_BIT_TASK_HCI);
            sys_task_delay(TASK_DELAY_MS + 4);
            Lpm_Low_Power_Unmask(LOW_POWER_MASK_BIT_TASK_HCI);
        }
...
void pci_rx_event(ruci_para_cnf_event_t *pci_cnf_event_ptr, uint8_t cmd_length)
{
    ...
    if (PCI_CMD_COMPLETE_EVENT(pci_cnf_event_ptr))
    {
        // sys_sem_signal(&g_tx_cmd_sem);
        notify_tx_task_tx_done(TX_TYPE_CMD);
    }
    ...
    if (msg_waiting_time != SYS_ARCH_TIMEOUT)
    {
        ...
        case ISR_MSG_TRAP:
        {
            ...
            for (j = 0; j < 5; j++)
            {
                ...
                for (i = 0; i < 0x100; i++)
                {
                    HPCI_PRINTF(COMMON_DEBUG_INFO, "%02x ", debug[i]);
                }
                HPCI_PRINTF(COMMON_DEBUG_INFO, "\n");
            }
        }
        break;
