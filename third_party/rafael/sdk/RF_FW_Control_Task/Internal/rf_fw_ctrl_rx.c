...
#define PCI_EVENT(event)\
        ...
        (event == RUCI_CMN_EVENT_HEADER)?TRUE:FALSE

#define PCI_CMD_COMPLETE_EVENT(data)  \
        ((((ruci_para_cnf_event_t*)data)->ruci_header.u8 == RUCI_PCI_EVENT_HEADER) && (((ruci_para_cnf_event_t*)data)->sub_header == RUCI_CODE_CNF_EVENT))?TRUE:\
        ((((ruci_para_cnf_event_t*)data)->ruci_header.u8 == RUCI_CMN_EVENT_HEADER) && (((ruci_para_cnf_event_t*)data)->sub_header == RUCI_CODE_GET_FW_VER_EVENT))?TRUE:FALSE
#endif
/**************************************************************************************************
 *    GLOBAL VARIABLES
 *************************************************************************************************/
...
void notify_tx_task_tx_done(uint8_t tx_type)
{
    ...
    (tx_type == TX_TYPE_DATA) ? sys_sem_signal(&g_tx_data_sem) :
...
void hci_rx_event(uint8_t *cmd_ptr, uint8_t cmd_length)
{
    ...
    hci_check_data_complete(p_hci_message->msg_type.hci_event.event_code,
                            p_hci_message->msg_type.hci_event.parameter);

    hci_check_cmd_complete(p_hci_message->msg_type.hci_event.event_code,
                           p_hci_message->msg_type.hci_event.parameter);
...
void pci_rx_data(uint8_t *rx_data_ptr, uint8_t *ctrl_field_ptr)
{
    ...
    rfb_interrupt_event->rx_done(ctrl_field.Length, rx_data_ptr,
...
void task_rx_handle(void)
{
    ...
    //RfMcu_IsrInit(rf_fw_isr_hander, 0);
