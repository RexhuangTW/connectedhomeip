...
SemaphoreHandle_t    gxSecure_Semaphore;

int32_t    test_count = 0;

#endif

uint32_t   crypto_firmware = 0;
...
void crypto_lib_init(void)
{
    ...
    /*this function should call only once --- we don't check this...*/
    gxSecure_Semaphore = xSemaphoreCreateBinary();

    /*
     *  xSecure_Semaphore can NOT be null... we allocate the semaphore
     * in System initial stage.. so it could not fail.
     */
    xSemaphoreGive(gxSecure_Semaphore);

    test_count = 0;
    crypto_firmware = 0;         /*unknow firmware*/
