...
#define CONFIG_SYS_TIMER_RUNNING_STACK_SIZE 256
...
#define SYS_TIMER_CHANGE_EXECUTE_MODE_FROM_ISR(timer, execute_mode)                                                                \
    timer_cmd_send(timer, SYS_TIMER_CMD_CHANGE_EXECUTE_MODE, execute_mode, 1)
...
static sys_err_t debug_timer_init(void)
{
    ...
    do
    {
        debug_timer = sys_timer_create("BUG_t", SYS_TIMER_SECOND_TO_TICK(1), 1, SYS_TIMER_PRIORITY_0,
                                       SYS_TIMER_EXECUTE_ONCE_FOR_EACH_TIMEOUT, NULL, debug_timer_cb);
...
static void timer_excute_cb_timer_cb(sys_timer_t * ptimer)
{
    ...
    if (ptimer->wating_for_execution)
    {
        ...
        if ((ptimer->receive_delete_cmd == 1) && (ptimer->wating_for_execution == 0))
        {
...
static sys_err_t timer_running_task_create(void)
{
    ...
    for (u32_idx = 0; u32_idx < SYS_TIMER_PRIORITY_MAX; u32_idx++)
    {
        ...
        t_task       = sys_task_new(task_name[u32_idx], (sys_thread_fn) gpt_timer_execute_handler[u32_idx], NULL,
                                    CONFIG_SYS_TIMER_RUNNING_STACK_SIZE, u32_priority);
        if (t_task == NULL)
        {
...
static void _timer_start(sys_timer_t * timer)
{
    ...
    else
    {
        ...
        list_for_each(timer_list, &g_timer_ctrl.working_list, sys_timer_t, list)
        {
            ...
            else if ((timer_list->timeout == timer->timeout) && (timer_list->priority < timer->priority))
            {
                list_attach_prev(&timer_list->list, &timer->list);
                break;
            }
...
static void switch_to_running_task(sys_timer_t * timer)
{
    ...
    if ((timer->execute_mode == SYS_TIMER_EXECUTE_ONCE_FOR_DUPLICATE_TIMEOUT) && (timer->wating_for_execution > 0))
    {
        // running event already in running task queue
    ...
    if (sys_queue_send_with_timeout(&g_timer_running_queue[timer->priority], (void *) &timer, SYS_TIMER_CMD_QUEUE_SEND_TIMEOUT) !=
        ERR_OK)
    {
        timer->wating_for_execution--;
        TIMER_ERR_LOG();
...
static void sys_timer_isr(uint32_t timer_id)
{
    ...
    if ((g_timer_ctrl.current_time >= g_timer_ctrl.alarm_time) || (g_timer_ctrl.current_time > SYS_TIMER_SHIFT_THRESHOLD))
    {
...
sys_err_t sys_timer_hw_init(uint32_t u32_timer_id)
{
    ...
    do
    {
        ...
        NVIC_SetPriority((IRQn_Type) (Timer0_IRQn + u32_timer_id), 4);
...
int sys_timer_init(uint32_t u32_hw_timer_id)
{
    ...
    do
    {
        ...
        t_task = sys_task_new("s_timer_h", (TaskFunction_t) timer_handler, NULL, CONFIG_SYS_TIMER_STACK_SIZE,
...
sys_timer_t * sys_timer_create(const char * name, uint32_t period, uint32_t auto_reload, sys_timer_pri_t priority,
                               uint32_t execute_mode, void * cb_param, sys_timer_cb cb_function)
{
