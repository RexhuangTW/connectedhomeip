...
constexpr size_t kMaxNumberOfKeys  = 0x1E;
...
uint16_t GetStringKeyId(const char * key, uint16_t * freeId)
{
    ...
    for (keyId = 0; keyId < kMaxNumberOfKeys; keyId++)
    {
        ...
        err =
            chip::DeviceLayer::Internal::RT582Config::ReadConfigValueStr(pdmInternalId, keyString, kMaxKeyValueBytes, keyStringSize);
...
CHIP_ERROR KeyValueStoreManagerImpl::_Delete(const char * key)
{
    ...
    if (keyId < kMaxNumberOfKeys)
    {
        ...
        //ChipLogProgress(DeviceLayer, "KVS, delete from flash the Matter key [%s] with PDM id: 0x%02x", key, pdmInternalId);
        ...
        {
            pdmInternalId = chip::DeviceLayer::Internal::RT582ConfigKey(pdmIdKvsValue, keyId);
            //ChipLogProgress(DeviceLayer, "KVS, delete from flash the value of the Matter key [%s] with PDM id: 0x%02x", key,
            //                pdmInternalId);
            ...
            if (err != CHIP_NO_ERROR)
            {
                ...
                //                "KVS, Error while deleting from flash the value of the Matter key [%s] with PDM id: 0x%02x", key,
                //                pdmInternalId);
            }
        }
        else
        {
            //ChipLogProgress(DeviceLayer, "KVS, Error while deleting from flash the Matter key [%s] with PDM id: 0x%02x", key,
            //                pdmInternalId);
        }
    }
exit:
